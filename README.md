...
## 6 LD decay
The extent of linkage disequilibrium (LD) was estimated based on 100,000 SNP markers with TASSEL v5 using the squared correlation between markers, ![formula](https://render.githubusercontent.com/render/math?math=R_{2}), over a window size of 2,000 bp (Bradbury et al., 2007). We filtered the individuals out with a marker coverage below `0.6`. For this analysis, we required that every SNP marker was observed at least 80 times in each subpopulation. This filtering resulted in 1,243,604 SNP markers. Additionally, we thinned the data set by randomly sampling 10,000 markers per chromosome. This was done as: <br />
The calculation can be found in the script `XXX.bash`. <br /> <br /> 
LD decay was modeled with a nonlinear regression model as expected value ![formula](https://render.githubusercontent.com/render/math?math=N_{m}) under drift-recombination equilibrium as <img src="https://render.githubusercontent.com/render/math?math=E_{(r_{^2})}=1/((1+C))"> and <img src="https://render.githubusercontent.com/render/math?math=C=4Nc"> with <img src="https://render.githubusercontent.com/render/math?math=N"> as the number of individuals at each site and C as the recombination coefficient between sites (Remington et al., 2001). The calculation can be found in the script `XXX.R`. We also created a pairwise LD heatmap with the R package LDheatmap (Shin et al., 2006). This calculation is contained in the `XXX.R` script. <br /><br />
...
The function for the calculation of the **<img src="https://render.githubusercontent.com/render/math?math=F_{ST}"> leveraging replicated selection** is contained in the `FstSum_value_calculation.R` script. <br /> <br /> 
...
![Figure_4](https://github.com/MilaTost/ExpEvo_with_replicated_selection/assets/63467079/663ee6e3-7891-4fd9-9aef-7f8a470a7284)<br /> 
...
![Figure_5_1](https://github.com/MilaTost/ExpEvo_with_replicated_selection/assets/63467079/76e648e6-fd51-41ab-bc79-6a9a201b0c80) <br /> 
